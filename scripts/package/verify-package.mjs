...
await cp(packed, app, { recursive: true, verbatimSymlinks: true });
...
await writeFile(
  probe,
  [
    // Resolution through the relinked @darts/*, and type stripping of a file
    // reached through one of those links: the two things packaging breaks.
...
try {
  ...
  throw new Error(
    `the packed app does not run.\n\n${detail}\n\n` +
      `If this is ERR_MODULE_NOT_FOUND, electron-builder pruned the @darts/* links again ` +
      `and scripts/package/afterPack.cjs did not put them back.`,
  );
} finally {
